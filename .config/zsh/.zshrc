...
### Set variables
#################
PATH="/home/vic/bin:/usr/local/bin:/usr/local/sbin/:$PATH"
...
function b64link() { xdg-open $(echo $1 | base64 -d -) }
...
function importhydown () {
    ...
    if [[ -z "$1" ]]; then
        ...
        poetry run hydownloader-importer run-job --path /mnt/Yukari/import/hydownloader --config hydownloader-import-jobs.py --job default  --subdir "$1" --verbose
    fi
    cd ~-
}
...
function ffmpegclip () {
    ...
    local from="$2"
    local to="$3"
    ffmpeg -i $1 -ss $from -to $to -c:v copy -c:a copy output.mp4
}

function ffmpegclipsubtitles () {
    local inputfile="$1"
    local from="$2"
    local to="$3"
    ffmpeg -i $1 -ss $from -to $to -c:v copy -copyts -c:a copy -vf subtitles=$1 output.mp4
}

function ffmpegtranspile() {
    local inputfile="$1"
    local codec="$2"
    local encoder="hevc_vaapi"
    case "$codec" in
        hevc)
            encoder="hevc_vaapi"
            ;;
